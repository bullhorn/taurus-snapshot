...
function parse(tokens) {
    ...
    for (const token of tokens) {
        switch (token) {
            ...
            case ')':
                const tmp = stack.pop() || [];
                const lastKey = tmp.pop() || '';
                tmp.push([lastKey, curr]);
                curr = tmp;
                break;
            default:
                curr.push(token);
...
function missingSubFields(field, meta) {
...
function filterNotInMeta(fields, meta) {
    return fields
        .map((field) => {
        const [key, children] = fieldTuple(field);
            ...
            for (const field of result.fields) {
                ...
                if (!exists) {
                    ...
                    for (const assocEntityField of field.associatedEntity.fields) {
                        const assocEntityFieldExists = exists.associatedEntity.fields.find((f) => f.name === assocEntityField.name);
